...
SELECT  
	[Impact] = CAST((avg_total_user_cost * avg_user_impact) * (user_seeks + user_scans) AS bigint),  
	ImpactScale = LEN(CAST(CAST((avg_total_user_cost * avg_user_impact) * (user_seeks + user_scans) AS bigint) AS NVARCHAR(100))),
	user_seeks,
	[Table] = [statement],
